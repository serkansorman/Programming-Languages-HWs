...
(defun insert-n (lst number index)
	(cond 
  		((< index 0) lst) ;Invalid index
  		((= index 0) (cons number lst)) ;; if index is 0, push it to head of list
  		((cons (car lst) (insert-n (cdr lst) number (- index 1)))) ;; else shift list until reach the index 
  	)
)
